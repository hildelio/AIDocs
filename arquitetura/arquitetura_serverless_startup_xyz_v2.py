# Arquitetura Serverless - Startup XYZ (Com IA)
...
from diagrams.aws.ml import Textract # <-- Importando o serviço de IA
...
with Diagram(
    "Arquitetura Serverless - Startup XYZ (Com Extração de IA)",
    direction="LR",
    show=False,
    outformat="png",
    ...
    node_attr=node_attr,
    edge_attr=edge_attr,
    filename="arquitetura_serverless_startup_xyz_v5_IA",
) as diag:
    ...
    with Cluster("AWS Cloud - Backend Serverless", graph_attr=cluster_attr):
        ...
        s3 = S3("Amazon S3\n(Arquivamento 365 dias)")

    # Fluxo da arquitetura com rótulos explicativos (O Lambda agora chama o Textract antes do S3)
    cliente >> Edge(label=" Requisição\ncom PDF e ID ") >> api_gateway
    api_gateway >> Edge(label=" Aciona ") >> lambda_func
    
    # O Lambda interage com a IA e depois salva no Storage
    lambda_func >> Edge(label=" 1. Extrai Dados ") >> textract
    lambda_func >> Edge(label=" 2. Salva PDF e\nTexto Extraído ") >> s3